            ...
            <div class="d-flex align-items-center">
                ...
                <ul class="navbar-nav mb-2 mb-lg-0 ml-auto">
                    ...
                    <form method="GET" action="index.php">
                        <input class="form-control mr-sm-2" type="search" placeholder="Search" aria-label="Search" name="u" id="searchInput" autocomplete="off">
...
/* Styling the dropdown divider */
.dropdown-divider {
    border-color: #e9ecef; /* Light gray divider color */
}

/* Styling the dropdown toggle link */
.dropdown-toggle::after {
    content: ""; /* Hide default caret */
    display: inline-block;
    width: 0;
    height: 0;
    border-style: solid;
    border-width: 5px 5px 0 5px; /* Triangle caret */
    border-color: #333333 transparent transparent transparent; /* Triangle color */
    margin-left: 5px; /* Space between text and caret */
}

/* Styling the dropdown toggle link on hover */
.dropdown-toggle:hover::after {
    border-color: #0056b3 transparent transparent transparent; /* Darker triangle color on hover */
}
...
.search-icon {
    cursor: pointer;
    padding: 38px 46px 16px; /* Adjust the padding values to move the icon higher */
    color: white;
    font-weight: bold; /* Optionally, retain the bold font weight */
}
...
searchInput.addEventListener('input', function() {
    ...
    if (searchQuery.length > 0) {
        fetch(`search_suggestions.php?query=${searchQuery}&type=username`)
            .then(response => response.json())
            .then(data => {
                const suggestions = data.suggestions;
                searchSuggestions.innerHTML = suggestions.map(suggestion => `<div class="suggestion">${suggestion}</div>`).join('');
                searchSuggestions.style.display = 'block'; // Show the suggestions
            })
            .catch(error => console.error('Error fetching suggestions:', error));
    } else {
        searchSuggestions.innerHTML = '';
        searchSuggestions.style.display = 'none'; // Hide the suggestions if input is empty
    }
});
...
searchSuggestions.addEventListener('click', function(event) {
    if (event.target.classList.contains('suggestion')) {
        ...
        const profileUrl = event.target.dataset.profileUrl;
